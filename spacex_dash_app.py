...
# Callback function for `site-dropdown` and `payload-slider` inputs, `success-payload-scatter-chart` output
...
def get_pie_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        pie_chart = px.pie(filtered_df, values = 'class' , names = 'Launch Site' )
    else:
    ...
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        title = f'Payload vs. Class for {entered_site}'
